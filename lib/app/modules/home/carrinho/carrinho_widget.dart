        ...
        children: <Widget>[
          ...
          Observer(
            builder: (_){
              ...
              List<EnderecoModel> listEnd = Modular.get<EnderecoController>().listaEndereco.data;

              return CustomComboboxWidget(
                itens: listEnd.map((data) => Model(data.id, data.descricao)).toList(),
                onChange: (model) => controller.setEndereco(EnderecoModel(id: model.id, descricao: model.descricao)),
...
//          Observer(
//            builder: (_){
//              if(controller.getFormaPagto() == null){
//                return Text("Nenhuma Forma de Pagamento Cadastrada!");
//              }
//
//              List<FormaPagtoModel> listForma = controller.listaFormaPagto.data;
//
//              return CustomComboboxWidget(
//                itens: listForma.map((data) => Model(data.id, data.nome)).toList(),
//                onChange: (model) => controller.setFormaPagto(FormaPagtoModel(id: model.id, nome: model.descricao)),
//                itemSelecionado: null,
